#code from https://github.com/Christophe-Foyer/GVF-Python
#Change a little for preventing divide 0
import numpy as np
from scipy.ndimage.filters import laplace as del2
RE_VALUE = 1e-10 # By Xin Chen 3/30/2022
def GVF(f, mu, ITER):
    """
    ...
    fmin  = np.min(f[:, :]);
    fmax  = np.max(f[:, :]);
    f = (f-fmin)/(fmax-fmin+RE_VALUE);  #% Normalize f to the range [0,1] #By Xin chen 3/30/2022
    ...
    #   print(1, '%3d', i);  #commend by Xin Chen 2022/3/29
    #   if (i%20 == 0):
    #      print(1, '\n');
